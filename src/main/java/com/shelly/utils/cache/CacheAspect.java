...
public class CacheAspect {
    ...
    @Pointcut("@annotation(com.shelly.utils.cache.Cache)")
    ...
    public Object around(ProceedingJoinPoint joinPoint) {
        ...
        if (StringUtils.isNotBlank(json)) {
            ...
            try {
                ...
                throw new ServiceException("缓存错误");
            }
        }
        if (json != null) {
            // 空数据
    ...
    private Method getRealMethod(ProceedingJoinPoint joinPoint, Method method) {
        ...
        try {
            ...
            log.error("反射错误");
            throw new ServiceException("反射错误");
        }
        return realMethod;
    }
    ...
    private Map<String, Object> getParamMap(Method method, List<String> parameterNames, Object[] args) {
        ...
        if (paramMap.isEmpty()) {
            throw new ServiceException("缓存的key未正常配置！");
        }
        ...
        for (int i = 0; i < parameterAnnotations.length; i++) {
            ...
            for (Annotation annotation : parameterAnnotations[i]) {
                if (annotation instanceof CacheParam cacheParam && !cacheParam.value().isEmpty()) {
                    ...
                    if (!matchFound) {
                        throw new ServiceException("参数名 '" + cacheParamName + "' 未找到对应匹配的方法参数名！");
                    } else {
    ...
    private String generataKey(String paramKey, Map<String, Object> paramMap, RedisConstants constants) {
        ...
        while (matcher.find()) {
            ...
            if(paramValue == null && !paramMap.containsKey(paramName)) {
                throw new ServiceException("参数名 '" + paramName + "' 未找到对应匹配的方法参数名！ ");
            }
            // 拼接key
            if (!isFirst) {
                builder.append(":");
